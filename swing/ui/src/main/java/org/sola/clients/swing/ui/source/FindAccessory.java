...
/**
 * A threaded file search accessory for JFileChooser. <P> Presents JFileChooser
 * users with a tabbed panel interface for specifying file search criteria
 * including (1) search by name, (2) search by date of modification, and (3)
 * search by file content. Finded are performed "in the background" with found
 * files displayed dynamically as they are found. Only one search can be active
 * at a time. FindResults are displayed in a scrolling list within a results tab
 * panel. <P> Findes are performed asynchronously so the user can continue
 * browsing the file system. The user may stop the search at any time. Accepting
 * or cancelling the file chooser or closing the dialog window will
 * automatically stop a search in progress. <P> The starting folder of the
 * search (the search base) is displayed at the top of the accessory panel. The
 * search base dsiplay will not change while a search is running. Thes search
 * base display will change to reflect the current directory of JFileChooser
 * when a search is not running. <P> Changing the search options does not affect
...
public class FindAccessory extends JPanel
     ...
     * equals <b>maxMatches</b>. <P> <b>Note:</b> Convert this to a nonrecursive
     * search algorithm on systems where stack space might be limited and/or the
     * search hierarchy might be very deep.
     *
...
/**
 * Implements user interface and generates FindFilter for selecting files by
 * content. <P> <b>WARNING:</B> The FindFilter inner class for this object does
 * not implement an efficient strng search algorithm. Efficiency was traded for
 * code simplicity.
 */
